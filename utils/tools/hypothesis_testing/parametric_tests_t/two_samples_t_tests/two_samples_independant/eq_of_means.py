...
from utils.tools.hypothesis_testing.parametric_tests_t.two_samples_t_tests.two_samples_independant.eq_of_variances import eq_of_variances
import os # type: ignore
import datetime #type: ignore

def eq_of_means(df, col1, col2):
    ...
    between_directory_2 = os.path.join(between_directory_3, 'equality_of_means')
    subdirectory = os.path.join(between_directory_2, f'eq_of_means_{timestamp}')
    ...
    report_eq_var, reject_null_eq_var = eq_of_variances(alpha, array_list, reject_null_norm, samplesize1)

    """Perform a t-test on two datasets."""
    if reject_null_eq_var :
        t_stat, p_value = stats.ttest_ind(array_list[0], array_list[1], equal_var=True)
    else : 
        t_stat, p_value = stats.ttest_ind(array_list[0], array_list[1], equal_var=False)
    ...
    report = f"""
        ...
        Alpha: {alpha}
        Method Used: {'Student\'s T test, 2 samples independant' if reject_null_eq_var else 'Welch\'s T test'}
    ...
    plt.title(f'Independant equality of means between {col1} and {col2}')
    plt.grid(True, alpha=0.3)

    # Save first boxplot
    filename1 = os.path.join(subdirectory, f"boxplot_independant_eq_of_means_{col1}_&_{col2}_{timestamp}.png")
    ...
    feedback_file = os.path.join(subdirectory, f'Independant_eq_of_means_report_{timestamp}.txt')
